...
void solve()
{
    ...
    // for (auto& x : pre) x = ranges::lower_bound(tmp,x) - tmp.begin() + 1;
    int m = tmp.size();
    FenwickTree tr(m);
    ll ans = 0;
    for (ll x : pre) 
    {
        ans += tr.query(ranges::lower_bound(tmp,x - t + 1) - tmp.begin() + 1,m);
        int pos = ranges::lower_bound(tmp,x) - tmp.begin() + 1;
        tr.update(pos,1);
    }
    cout << ans << endl;
}
