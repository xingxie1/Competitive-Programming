...
void solve()
{
    ...
    if (n == 1) 
    {
        ll ans = 3 * qpow(2,m - 1) % MOD;
        cout << ans << endl;
        return ;
    }
    int N = 1 << (n - 2);
    Matrix a(N,N),f(N,1);
    auto build = [&](int mask,int fi,int se) -> vt
    {
        vt col(n);
        col[0] = fi;
        col[1] = se;
        for (int i = 2;i < n;i++)
        {
            if ((mask >> (i - 2)) & 1) col[i] = col[i - 2];
            else 
            {
                int c1 = col[i - 1],c2 = col[i - 2];
                col[i] = 3 - c1 - c2;
            }
        }
        return col;
    ...
    {
        vt cur = build(mask,0,1);
        for (int nmask = 0;nmask < N;nmask++)
        {
    ...
    Matrix res = powMul(a,m - 1,f);
    ll ans = 0;
    for (int i = 0;i < N;i++) 
    {
        ans = (ans + 6 * res[i][0]) % MOD;
    }
    cout << ans << endl;
}
