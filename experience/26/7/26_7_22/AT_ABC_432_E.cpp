...
void solve()
{
    ...
    cin >> n >> q;
    vt a(n);
    FenwickTree<ll> cnt(5e5 + 5),sum(5e5 + 5);
    for (int i = 0;i < n;i++) 
    {
        cin >> a[i];
        sum.update(a[i] + 1,a[i]);
        cnt.update(a[i] + 1,1);
    }
    while (q--)
    {
        ...
        else 
        {
            ...
            {
                ll ans = sum.pre(r) - sum.pre(l + 1);
                ans += 1LL * l * cnt.pre(l + 1) + 1LL * r * (cnt.pre(5e5 + 4) - cnt.pre(r));
                cout << ans << endl;
            }
        }
    }
