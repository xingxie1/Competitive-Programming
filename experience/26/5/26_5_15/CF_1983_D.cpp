...
void solve()
{
    ...
    for (int& x : a) x = ranges::lower_bound(tmp1,x) - tmp1.begin() + 1;
    for (int& x : b) x = ranges::lower_bound(tmp2,x) - tmp2.begin() + 1;
    FenwickTree tr1(n),tr2(n);
    auto f = [&](vt& a,FenwickTree<int>& tr)
    {
        ll sum = 0;
        for (int x : a) 
        {
