...
void solve()
{
    ...
    for (int i = 0;i < n;i++) 
    {
        id[i] = ranges::lower_bound(tmp,a[i]) - tmp.begin();
    }
    int lim = min(n + 1,m + 1);
    ...
    for (int x = 1;x <= lim;x++)
    {
        ...
        for (int p : pos[x])
        {
            if (last + 1 <= p - 1)
            {
                b.push_back({last + 1,p - 1,x});
            }
            last = p;
        }
        if (last + 1 <= n - 1) b.push_back({last + 1,n - 1,x});
    }
    ranges::sort(b,{},[&](auto& y){
        return y.r;
    });
    FenwickTree tr(n);
    map<int,int> last;
    ...
    for (auto& [l,r,x] : b)
    {
        while (i <= r) 
        {
            ...
            if (last.count(c)) 
            {
                tr.update(last[c] + 1,-1);
            }
            last[c] = i;
            tr.update(i + 1,1);
            i++;
        }
        int cnt = tr.query(l + 1,r + 1);
        ans = max(ans,cnt - x);
    }
    cout << ans << endl;
}
